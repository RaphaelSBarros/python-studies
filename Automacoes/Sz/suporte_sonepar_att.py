...
for chamado in REPORT["B"][2:]:
    ...
    for atualizar in UPDATE["B"][1:]:
        if (atualizar.value == chamado.value
            ...
            final_ws.cell(row=new_row, column=9, value=UPDATE[f"H{atualizar.row}"].value)
            final_ws.cell(row=new_row, column=10, value=UPDATE[f"I{atualizar.row}"].value)
...
final_wb.save(f"data/suporte_sonepar_final_{yesterday}.xlsx")
