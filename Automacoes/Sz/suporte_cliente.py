...
for cliente in relatorio["C"][1:]:
    ...
    if cliente.value == "SZ Soluções":
        ...
        int_final_ws.cell(row=int_new_row, column=4, value= "algo")
        int_final_ws.cell(row=int_new_row, column=5, value= relatorio[f"E{cliente.row}"].value)
        ...
        int_final_ws.cell(row=int_new_row, column=10, value= datetime.datetime(year=int(fdate[2]), month=int(fdate[1]), day=int(fdate[0]), hour=int(ftime[0]), minute=int(ftime[1]), second=int(ftime[2])) - datetime.datetime(year=int(idate[2]), month=int(idate[1]), day=int(idate[0]), hour=int(itime[0]), minute=int(itime[1]), second=int(itime[2])) )
    else:
        ...
        ext_final_ws.cell(row=ext_new_row, column=10, value= datetime.datetime(year=int(fdate[2]), month=int(fdate[1]), day=int(fdate[0]), hour=int(ftime[0]), minute=int(ftime[1]), second=int(ftime[2])) - datetime.datetime(year=int(idate[2]), month=int(idate[1]), day=int(idate[0]), hour=int(itime[0]), minute=int(itime[1]), second=int(itime[2])) )
